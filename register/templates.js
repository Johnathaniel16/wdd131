  ...
  return `
    <section class="participant participant${count}">
      ...
      <div class="item">
        ...
        <input id="fee${count}" type="number" name="fee${count}" min="0" step="1" required />
      </div>
...
export function successTemplate(info) {
  const feeText = Number(info.total).toFixed(2);
  return `Thank you ${info.name} for registering. You have registered ${info.count} participants and owe $${feeText} in Fees.`;
}
